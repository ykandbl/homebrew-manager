 ...
 * 根据过滤器筛选包列表
 */
export function filterPackages(packages: Package[], filter: PackageFilter): Package[] {
  switch (filter) {
    case 'all':
      return packages;
    case 'formula':
 ...
 * 计算各过滤器的包数量
 */
export function getFilterCounts(packages: Package[]): Record<PackageFilter, number> {
  return {
    all: packages.length,
